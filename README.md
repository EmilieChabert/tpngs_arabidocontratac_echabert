...
Données d'entrée: données trimmées pairés (trimmed.fastq) + génome indexé
...
1. Marquer les duplicats de PCR parmi les reads mappés avec $PICARD MarkDuplicates
...
Fonction utlisée et ouutput: samtools view (fonction permettant de manipuler les fichiers BAM et d'enlever ou conserver certains flags, ex:1024 = reads dupliqués) --> fichier BAM contenant les reads uniques mappant correctement sur le génome nucléaire de A.thaliana


# Contrôle qualité spécifique à l'ATAC-seq

Cette analyse est spécifique de l'ATAC-seq. On la réalise pour montrer que la manipulation d'ATAC-seq a été techniquement correctement réalisé.
...
1. Création d'un fichier contenant les coordonnées des intervalles de 1000 bp autour des TSS en utilisant grep et bedtools = chromosome, intervalle, gène associé au TSS, direction du gène
2. Identification des reads mappant pour chaque position de l'intervalle

Données d'entrée: reads filtrés + fichier cotenant les intervalles de 1000 bp autour du TSS

Fonction utilisée: bedtools coverage --> fichier donnant pour chaque intervalle le nombre de reads mappant pour chaque position de l'intervalle
...
La visualisation des pics (fichier .broadPeak) se fait sur IGV (Integrative Genomics Viewer). Télécharger également les données filtrées au préalable indexer pour visualiser les reads et leurs fréquences sur le génome
...
Pour la suite de l'analyse nous avons choisi de sélectionner uniquement les pics présent dans au moins deux sur trois des sets de données sur les cellules quiescentes en utilisant le script Analyse_compatison.R
